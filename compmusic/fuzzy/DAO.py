...
class DAO:
    ...
    def getRecordingInfo(self, attr_name, attr_value):
        ...
        if status:
            ...
            # get artist relations: mainly the instrument or vocals
            status, artistrel, msg = self.getArtistsRelation("uuid", recording['uuid'], rel_table="recording",
                                                             relation=None)
            if status:
                recording['artists'] = [artist for artist in artistrel]
            # get work
    ...
    def getReleaseInfo(self, attr_name, attr_value):
        ...
        if status:
            # get artist relations: mainly the instrument or vocals
            status, artistrel, msg = self.getArtistsRelation("uuid", release['uuid'], rel_table="release",
                                                             relation=None)
            if status:
                release['artists'] = [artist for artist in artistrel]
        return release
    ...
    def getWorkRelationsInfo(self, relation, collection=None):
        status, relations, msg = self.getArtistsRelations(rel_table="work", relation=relation,
                                                          collection=collection)  # TODO: ArtistsRelations
        finalrelations = []
        if status:
...
if __name__ == "__main__":
    ...
    if status:
        for recording in recordings:
            ...
            status, artistrel, msg = dao.getArtistsRelation("uuid", recording['uuid'], rel_table="recording",
                                                            relation=None)
            if status:
                recording['artists'] = [artist for artist in artistrel]
            # get work
