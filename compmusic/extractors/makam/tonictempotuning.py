...
class TonicTempoTuning(compmusic.extractors.ExtractorModule):
    ...
    def run(self, musicbrainzid, fname):
        ...
        symbtr = dunya.makam.get_symbtr(rec_data['works'][0]['mbid'])
        if not symbtr['path'] or symbtr['path'] == '':
            raise Exception('No work on recording %s' % musicbrainzid)
        
        symbtrtxt = symbtr['path'] 
        mp3file = fname
        mlbinary = '/srv/dunya/predominantMelody.mat'
        output = tempfile.mkdtemp()
       
        proc = subprocess.Popen(["/srv/dunya/extractTonicTempoTuning %s %s %s %s" % (symbtrtxt, mp3file, mlbinary, output)], stdout=subprocess.PIPE, shell=True, env=subprocess_env)
